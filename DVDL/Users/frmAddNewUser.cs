...
namespace DVDL.Users
{
    public partial class frmAddNewUser : Form
    {
        public frmAddNewUser()
        {
            ...
            tabUserDetails.PageEnabled = false;
        }
        ...
        private void txtFilterBy_TextChanged(object sender, EventArgs e)
        {
            if (!string.IsNullOrEmpty(txtFilterBy.Text.ToString()))
            {
                ...
                if (drPerson != null)
                {
                    tabUserDetails.PageEnabled = true;
                }
                else
                {
                    tabUserDetails.PageEnabled = false;
        ...
        private void btnAddUser_Click(object sender, EventArgs e)
        {
            ...
            if (clsBusiness.clsUsers.GetUserDetailsWithPersonID(PersonID) == null)
            {
                if (MessageBox.Show($"Are you sure you want to add this User ? ", "Confirm ? ",
                    MessageBoxButtons.YesNo, MessageBoxIcon.Information) == DialogResult.Yes)
                {
                    if ((UserID = clsBusiness.clsUsers.AddNewUser(PersonID, ctrlAddUser.Username, ctrlAddUser.ConfirmPassword, ctrlAddUser.Status)) != -1)
                    {
                        MessageBox.Show($"User With ID : [{UserID}] Added Successfully.", "Added Successfully",
                            MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
                    else if (clsBusiness.clsUsers.CheckIfUsernameIsExist(ctrlAddUser.Username.Trim())) 
                    {
                        MessageBox.Show("User With this Username is already Exist !", "Invalid Username",
                            MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
                    else
                    {
                        MessageBox.Show("User was not Added !", "Addition Failed",
                            MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
                }
                else
            ...
            {
                MessageBox.Show("User With this Person ID is already User !", "Invalid User",
                    MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
